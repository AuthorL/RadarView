...
class RadarView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet, defStyleAttr: Int = 0) : View(context, attributeSet, defStyleAttr) {
     ...
     * 分割线颜色
     * */
    internal var lineColor = Color.parseColor("#D81B60")
        set(value) {
            field = value
    ...
    internal var data: Map<String,Float> = emptyMap()
        set(value) {
    ...
    private var keyList:List<String> = data.keys.toMutableList()
    private var valueList:List<Float> = data.values.toMutableList()
    /**
     * 维度
     * */
    ...
    private fun Canvas.drawDesc() {
        Path().apply {
            val textHeight = textPaint.fontMetrics.let {
                it.descent - it.ascent.also { h -> Log.d(MainActivity.TAG, "文字高度为${abs(h)}") }
            }
...
//            alpha = 50
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        radius = w.coerceAtMost(h) / 2 * 0.8.toFloat()
        viewWidth = w
        viewHeight = h
